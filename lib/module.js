...
module.exports = async function (moduleOptions) {
  ...
  return async (req, res) => {
    const feedIndex = options.findIndex(
      (o) => o.path === req.url.replace(/\/$/, "")
    );
    if (feedIndex < 0) {
    ...
    try {
      const xml = await feedCache.get(feedIndex);
      res.set("Content-Type", resolveContentType(options[feedIndex].type));
      res.send(xml);
    } catch (err) {
      console.error(err);
